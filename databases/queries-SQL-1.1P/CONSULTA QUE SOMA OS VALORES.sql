SELECT DISTINCT
	...
	CASE WHEN DATEDIFF(MONTH, TMOV.DATACRIACAO, MOVIMENTO_DESTINO.DATAEMISSAO) <> 0 THEN '0'
		...
		CASE WHEN TITMMOV.QUANTIDADE <> '0' THEN TITMMOV.QUANTIDADE
		WHEN TITMMOV.QUANTIDADE = '0' THEN TITMMOV.QUANTIDADETOTAL END as 'QUANTIDADE',
	...
	*/
	...
	(TITMMOV.PRECOUNITARIO + (CASE WHEN TCORCAMENTO.DESPESA <> 0 THEN TCORCAMENTO.DESPESA ELSE '0' END / CASE WHEN TITMMOV.QUANTIDADE <> 0 THEN TITMMOV.QUANTIDADE ELSE '1' END)) AS 'VALOR ITM COM FRETE',
...
AND TMOV.DATACRIACAO BETWEEN '01-06-2021' AND '15-07-2021'
AND TMOV.IDMOV = 13217
...
--WHERE TMOV.SERIE IN ('SC') AND TMOV.DATACRIACAO BETWEEN :DATA_INICIAL and :DATA_FINAL AND (MOVIMENTO_DESTINO.DATALANCAMENTO not BETWEEN :DATA_INICIAL and :DATA_FINAL OR MOVIMENTO_DESTINO.IDMOV IS NULL ) AND TMOV.STATUS <>'C'
...
--WHERE TMOV.SERIE IN ('OC','OCM') AND TMOV.DATAEMISSAO BETWEEN :DATA_INICIAL AND :DATA_FINAL AND TMOVRELAC.IDMOVORIGEM IS NULL AND TMOV.STATUS <> 'C'
...
--WHERE TABELA.[VENCEDOR] = 1 AND TABELA.[STATUS] <> 7 AND TABELA.[DATA COTACAO] BETWEEN :DATA_INICIAL AND :DATA_FINAL
...
--AND TMOV.SERIE IN ('SC') AND TCITMORCAMENTO.CFOVENCEDOR <> 0 AND TMOV.DATACRIACAO not BETWEEN :DATA_INICIAL and :DATA_FINAL AND MOVIMENTO_DESTINO.DATACRIACAO BETWEEN :DATA_INICIAL and :DATA_FINAL AND TCITMORCAMENTO.CFOVENCEDOR = 1
--AND (SELECT F.PRECOUNITARIO FROM TITMMOV F WHERE F.CODCOLIGADA = MOVIMENTO_DESTINO.CODCOLIGADA AND F.IDMOV = MOVIMENTO_DESTINO.IDMOV AND F.IDPRD = TITMMOV.IDPRD and TCCOTACAO.STSCOTACAO <> 7) IS NOT NULL  
