...
/*********DISCIPLINAS EQUIVALENTES A DISCIPLINAS DA GRADE NORMAL CONCLUIDAS****/
	SELECT DISTINCT
		...
		SHISTDISCEQUIVCONCLUIDAS.CODDISC				[CODIGO DISCIPLINA],
		...
		STATUSCURSO.DESCRICAO							[STATUS_CURSO]
	...
	FROM SHISTDISCEQUIVCONCLUIDAS (NOLOCK)
		...
		INNER JOIN SSTATUS STATUSCURSO (NOLOCK) ON
				STATUSCURSO.CODSTATUS = SHABILITACAOALUNO.CODSTATUS
	...
	WHERE SHABILITACAOALUNO.CODSTATUS <> 18 /*TODOS OS ALUNOS EXCETO GRADUADOS*/
...
UNION ALL
	...
	SELECT DISTINCT
		...
		SHABILITACAOALUNO.CODSTATUS,
		SMODALIDADECURSO.DESCRICAO		[MODALIDADE CURSO],
		SHISTORICO.CODDISC				[CODIGO DISCIPLINA],
...
UNION ALL
	...
	SELECT DISTINCT
		...
		SHABILITACAOALUNO.CODSTATUS,
		SMODALIDADECURSO.DESCRICAO		[MODALIDADE CURSO],
		SHISTDISCPENDENTES.CODDISC		[CODIGO DISCIPLINA],
...
SELECT DISTINCT
	...
	SHABILITACAOALUNO.CODSTATUS,
	SMODALIDADECURSO.DESCRICAO										[MODALIDADE CURSO],
	SHISTDISCOPTELETIVAS.CODDISC									[CODIGO DISCIPLINA],
	SHISTDISCOPTELETIVAS.PLETIVO									[PERIODO LETIVO],
	SHISTDISCOPTELETIVAS.DISCIPLINA									[DISCIPLINA],
	0																[PERIODO],
	CASE 
		WHEN SHISTDISCOPTELETIVAS.STATUS IS NULL THEN	'Pendente'
		ELSE SHISTDISCOPTELETIVAS.STATUS
	END																[STATUS],
	STATUSCURSO.DESCRICAO											[STATUS_CURSO]
FROM SHISTDISCOPTELETIVAS (NOLOCK)
	INNER JOIN SHABILITACAOALUNO (NOLOCK) ON
			SHABILITACAOALUNO.IDHABILITACAOFILIAL = SHISTDISCOPTELETIVAS.IDHABILITACAOFILIAL
		AND SHABILITACAOALUNO.RA = SHISTDISCOPTELETIVAS.RA	
	INNER JOIN SSTATUS STATUSCURSO (NOLOCK) ON
			STATUSCURSO.CODSTATUS = SHABILITACAOALUNO.CODSTATUS
	INNER JOIN SALUNO (NOLOCK) ON
			SALUNO.CODCOLIGADA = SHABILITACAOALUNO.CODCOLIGADA
