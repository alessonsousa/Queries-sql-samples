...
TABELA.[VALOR UNITARIO] + CONVERT(DECIMAL(10,4), ROUND((TABELA.[FRETE]/
...
CONVERT(DECIMAL(10,4), TABELA.[QTD]) as 'QTD',
(TABELA.[VALOR UNITARIO] + CONVERT(DECIMAL(10,4), ROUND((TABELA.[FRETE]/
(SELECT COUNT (*) FROM (SELECT DISTINCT IDMOV FROM TCITMORCAMENTO ITMORC WHERE ITMORC.CODCOTACAO = TABELA.[COD. COTA플O]) ABC))/
CONVERT(INTEGER, SUM(TABELA.QUANTIDADENEG)),2,0))) * TABELA.[QTD] AS 'VALOR TOTAL',
TABELA.[DATA COTACAO],
...
TABELA.[DESPESA],
CONVERT(DECIMAL(10,4), ROUND((TABELA.[FRETE]/
(SELECT COUNT (*) FROM (SELECT DISTINCT IDMOV FROM TCITMORCAMENTO ITMORC WHERE ITMORC.CODCOTACAO = TABELA.[COD. COTA플O]) ABC))/
CONVERT(INTEGER, SUM(TABELA.QUANTIDADENEG)),2,0)) as 'FRETE POR ITEM',

CONVERT(DECIMAL(10,4),TABELA.[FRETE]/(SELECT COUNT (*) FROM (SELECT DISTINCT IDMOV FROM TCITMORCAMENTO ITMORC WHERE ITMORC.CODCOTACAO = TABELA.[COD. COTA플O]) ABC)) AS 'FRETE',
...
AND TABELA.[DATA COTACAO] >= '01/08/2021' AND TABELA.[DATA COTACAO] <= '31/08/2021'
